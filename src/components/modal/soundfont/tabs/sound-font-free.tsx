...
import { saveSoundFontStorage } from '@/lib/storage/soundfont';
import { FaCircleCheck } from 'react-icons/fa6';

interface SoundfontFreeProps {
    soundFontStorage: ListItem<IDBValidKey>[];
    getSoundFontList: () => Promise<ListItem<IDBValidKey>[]>;
}

const SoundfontFree: React.FC<SoundfontFreeProps> = ({ soundFontStorage, getSoundFontList }) => {
    ...
    const handleDownload = async (value: string, fileName: string) => {
        ...
        if (download) {
            await saveSoundFontStorage(download);
            getSoundFontList()
        }
    }
        ...
        <>
            <div className="flex flex-col gap-2 w-full h-full">
                <Label
                    ...
                    description={<span>ดาวน์โหลด Soundfont ฟรีคุณภาพสูงเพื่อใช้ในการเล่นดนตรี</span>}
                >
                        ...
                        return (
                            <>
                                <Button
                                    ...
                                    className="w-7 h-7"
                                    onClick={() => !isDownloaded ? handleDownload(value, name) : undefined}
                                    color="default"
                                    blur={false}
                                    icon={isDownloaded ? <FaCircleCheck className="text-green-500"></FaCircleCheck> : <BiDownload />}
                                    iconPosition='left'
                                /></>
                        );
                    }}
