...
interface NoteChildProps {
  on: SynthControl<INoteState, INoteChange>[];
  off: SynthControl<INoteState, INoteChange>[];
}
...
const NoteChild: React.FC<NoteChildProps> = ({ on, off }) => {
  ...
  useEffect(() => {
    ...
    on.forEach((node) => node.on(["NOTE_ON", "CHANGE"], onHandler, id));
    off.forEach((node) =>
      node.on(["NOTE_OFF", "CHANGE"], offHandler, id)
    );

    return () => {
      on.forEach((node) => node.off(["NOTE_ON", "CHANGE"], id));
      off.forEach((node) => node.off(["NOTE_OFF", "CHANGE"], id));
    };
  }, [on, off, id]);
