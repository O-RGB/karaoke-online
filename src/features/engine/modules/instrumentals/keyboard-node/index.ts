...
export class KeyboardNode {
  ...
  public event: EventManager<INoteState, TEventType<INoteChange>>[] = [];
  ...
  constructor(channel: number, keyModifierManager: KeyModifierManager) {
    ...
    this.notes = MAX_CHANNEL.map((_, midiNote) => {
      const noteEvent = new EventManager<INoteState, TEventType<INoteChange>>();
      this.event.push(noteEvent);
      return new SynthNode(noteEvent, "NOTE_ON", channel, {
        channel,
        midiNote,
        velocity: 0,
      });
